...
A(lin_ind_fill) = A(lin_ind_fill)' + lambda * smooth_weights(lin_ind(valid_ind_left),1); % edge right of left neighbor

lin_ind_fill = ...
    sub2ind(mat_size, lin_ind_right(valid_ind_right), lin_ind_right(valid_ind_right));
A(lin_ind_fill) = A(lin_ind_fill)' + lambda * smooth_weights(lin_ind(valid_ind_right),2); % edge left of right neighbor

lin_ind_fill = ...
    sub2ind(mat_size, lin_ind_up(valid_ind_up), lin_ind_up(valid_ind_up));
A(lin_ind_fill) = A(lin_ind_fill)' + lambda * smooth_weights(lin_ind(valid_ind_up),3); % edge below upper neighbor

lin_ind_fill = ...
    sub2ind(mat_size, lin_ind_down(valid_ind_down), lin_ind_down(valid_ind_down));
A(lin_ind_fill) = A(lin_ind_fill)' + lambda * smooth_weights(lin_ind(valid_ind_down),4); % edge above upper neighbor
